...
class PointwiseConfig:
    ...
    routing_peak_normalization: str = "protocol_max_tiles"

    def validate(self) -> "PointwiseConfig":
        get_shape_protocol(self.shape_protocol)
        if self.routing_peak_normalization != "protocol_max_tiles":
            raise ValueError("routing_peak_normalization must be protocol_max_tiles")
        ...
        if self.interaction_mode != "residual_pointwise":
            raise ValueError("interaction_mode must be residual_pointwise")
        if self.dfg_representation != "route_expanded":
            raise ValueError("dfg_representation must be route_expanded")
        if self.dfg_message_mode != "dual_mean":
            raise ValueError("dfg_message_mode must be dual_mean")
        if self.dfg_pool_mode != "all":
            raise ValueError("dfg_pool_mode must be all")
        if self.dfg_summary_mode != "none":
            raise ValueError("dfg_summary_mode must be none")
        if not float(self.mapper_ii_ceiling).is_integer():
            raise ValueError("mapper_ii_ceiling must be an integer")
        if not 0.0 <= self.discrete_success_threshold <= 1.0:
            raise ValueError("discrete_success_threshold must be in [0, 1]")
        if self.discrete_ii_decision not in {"map", "round", "floor", "ceil"}:
            raise ValueError(
...
def pad_semantic_adjacency(
    graphs: Sequence[GraphData], device: Any,
) -> Tensor:
    """Pad the materialized dependency graph used by routing estimates."""
    _require_torch()
    if not graphs:
        raise ValueError("cannot batch an empty graph sequence")
    maximum_nodes = max(len(graph.node_types) for graph in graphs)
    ...
    for graph_index, graph in enumerate(graphs):
        for source, target in (
            ...
            if graph.semantic_edges is not None else graph.edges
        ):
            adjacency[graph_index, source, target] = 1.0
    return adjacency


def pad_node_feature_mask(
    graphs: Sequence[GraphData], feature_index: int, device: Any,
) -> Tensor:
    """Pad one binary node feature into a mask."""
    _require_torch()
    maximum_nodes = max(len(graph.node_types) for graph in graphs)
    mask = torch.zeros(
        (len(graphs), maximum_nodes), dtype=torch.bool, device=device,
    )
    for graph_index, graph in enumerate(graphs):
        for node_index, features in enumerate(graph.node_features):
            mask[graph_index, node_index] = features[feature_index] > 0.5
    return mask
        ...
        def __init__(self, config: PointwiseConfig = PointwiseConfig()) -> None:
            ...
            self.dfg_encoder = DirectedGraphEncoder(
                len(
                    ...
                    if route_expanded else DFG_NODE_FEATURE_NAMES
                ), hidden,
        ...
        def forward(
            ...
            if self.config.interaction_mode in {
                ...
                if self.config.dfg_representation == "route_expanded":
                    ...
                    routing_adjacency = pad_semantic_adjacency(
                        dfg_graphs, device,
                    )
                else:
                    dfg_interaction_mask = dfg_mask
                    routing_adjacency = dfg_adjacency
                ...
                (
                    ...
                    self._candidate_conditioned_interaction(
                        dfg_nodes, dfg_interaction_mask,
                        cgra_nodes, cgra_mask,
                    )
                )
                routing_context = (
                    self._routing_context(
                        cross_attention, routing_adjacency, cgra_graphs,
                    )
                    if self.config.interaction_mode in {
                        "routing_set_attention", "discrete_routing_set",
                        "discrete_pointwise", "residual_pointwise",
...
__all__ = [
    ...
    "censored_top1_metrics", "make_cgra_graph", "pointwise_loss",
